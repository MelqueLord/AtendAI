...
export function WhatsAppWorkspace({
  ...
  async function refreshMetaArea() {
    await Promise.all([loadSetupState(), onRefreshMetaState()]);
  }
      ...
      <section className="grid items-start gap-6 xl:grid-cols-[minmax(0,1.12fr)_minmax(330px,0.8fr)]">
        ...
        <div className="space-y-4">
          <ChannelOptionCard
            ...
            footnote={metaFootnote}
            ctaLabel={metaReady ? "Abrir na plataforma" : hasPreparedChannel ? "Testar conexao" : "Preparar canal"}
            onAction={metaReady ? handleOpenMetaChannel : hasPreparedChannel ? () => { void handleTestMetaConnection(); } : () => setShowSetupForm(true)}
            message={metaMessage}
          />
