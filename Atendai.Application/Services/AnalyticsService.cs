...
public sealed class AnalyticsService(IConversationRepository conversationRepository) : IAnalyticsService
{
    ...
    public async Task<AnalyticsOverviewResponse> GetOverviewAsync(Guid tenantId, CancellationToken cancellationToken = default)
    {
        ...
        var waitingHuman = conversations.Count(c => c.Status == ConversationStatus.WaitingHuman);
        ...
        var aiResolved = conversations.Count(c =>
            ...
            c.Status != ConversationStatus.WaitingHuman);
    ...
    private static List<AnalyticsDailyPoint> BuildDailySeries(List<Conversation> conversations)
    {
        ...
        for (var i = 0; i < 7; i++)
        {
            ...
            result.Add(new AnalyticsDailyPoint(
                day.ToString("MM-dd"),
                inDay.Count,
                inDay.Count(c => c.Status == ConversationStatus.WaitingHuman),
    ...
    private static bool HasSchedulingIntent(Conversation conversation)
    {
