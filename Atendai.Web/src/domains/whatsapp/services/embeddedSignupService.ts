...
export async function fetchEmbeddedSignupConfig(apiBase: string, token: string) {
  const response = await fetch(`${apiBase}/engagement/whatsapp/meta/embedded-signup/config`, {
    headers: { Authorization: `Bearer ${token}` }
  });

  if (!response.ok) {
    throw new Error(await extractError(response, "Nao foi possivel carregar a configuracao oficial da Meta."));
  }

  return (await response.json()) as MetaEmbeddedSignupConfig;
}

export async function completeEmbeddedSignup(apiBase: string, token: string, payload: CompleteMetaEmbeddedSignupPayload) {
  const response = await fetch(`${apiBase}/engagement/whatsapp/meta/embedded-signup/complete`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    throw new Error(await extractError(response, "Nao foi possivel concluir o cadastro incorporado."));
  }

  return (await response.json()) as MetaEmbeddedSignupResult;
}

export async function launchEmbeddedSignup(apiBase: string, token: string, draft: EmbeddedSignupDraft) {
  const config = await fetchEmbeddedSignupConfig(apiBase, token);
  ...
  return completeEmbeddedSignup(apiBase, token, {
    code,
