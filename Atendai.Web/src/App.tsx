...
import { createAttendanceHubConnection, type AttendanceRefreshPayload } from "./app/realtime/attendanceHub";
import type {
  AppPage,
  AnalyticsOverview,
  AuthResponse,
...
export function App() {
  ...
  }, [conversationsWithResolvedNames, queueFilter, search]);
  ...
  useEffect(() => {
    ...
    void loadConversationDetail(selectedId, auth.token);
  ...
  useEffect(() => {
    ...
    const refreshFromRealtime = async (payload?: AttendanceRefreshPayload) => {
      ...
      const conversationId = payload?.conversationId ?? selectedIdRef.current;
      if (conversationId) {
        await loadConversationDetail(conversationId, auth.token);
        await loadConversationNotes(conversationId, auth.token);
      }
    };
  ...
  async function loadConversations(token = auth?.token) {
    ...
    try {
      const [conversationsRes, qrSessionRes] = await Promise.all([
        fetch(`${apiBase}/conversations`, {
          headers: { Authorization: `Bearer ${token}` }
        }),
        fetch(`${apiBase}/engagement/whatsapp/web/session`, {
          headers: { Authorization: `Bearer ${token}` }
        }).catch(() => null)
      ]);
      ...
      const data = (await conversationsRes.json()) as Conversation[];
      const qrSession = qrSessionRes?.ok ? (await qrSessionRes.json()) as WhatsAppWebSessionState : null;
      const shouldHideQrHistory = qrSession?.isConfigured === true && qrSession.status !== "connected";
      const visibleConversations = shouldHideQrHistory
        ? data.filter((conversation) => conversation.transport !== "qr")
        : data;
      ...
      if (!visibleConversations.length) {
        setSelectedId("");
        setSelectedConversationDetail(null);
      } else if (!selectedId || !visibleConversations.some((item) => item.id === selectedId)) {
  ...
  async function loadAttendanceContactsIndex(token = auth?.token) {
    ...
    try {
      const res = await fetch(`${apiBase}/crm/contacts?pageSize=200`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      setContacts((await safeJson<Contact[]>(res)) ?? []);
    } catch {
      // Attendance still works with persisted conversation names if this lightweight contact index fails.
    }
  }

  async function loadConversationDetail(conversationId: string, token = auth?.token) {
  ...
  function mergeConversationIntoAttendanceState(updatedConversation: Conversation) {
    ...
    if (selectedIdRef.current === updatedConversation.id || selectedId === updatedConversation.id) {
      setSelectedConversationDetail(updatedConversation);
    }
  }
  ...
  async function refreshInboxOnly(token = auth?.token) {
    ...
    if (selectedId) {
      await loadConversationDetail(selectedId, token);
  ...
  async function loadCommercial(token = auth?.token, role = auth?.role) {
    ...
    try {
      ...
      if (!plansRes.ok || !subRes.ok || !metricsRes.ok) {
        const bodies = await Promise.all([plansRes, subRes, metricsRes].map(async (response) => `${response.url} -> ${response.status} ${await response.text()}`));
        setError(`Erro ao carregar dados comerciais: ${bodies.join(" | ")}`);
        return;
      }
  ...
  async function updateSelectedConversationStatus(
    ...
    try {
      ...
      const updatedConversation = await safeJson<Conversation>(res);
      ...
      if (selectedIdRef.current === conversationId) {
        void loadConversationDetail(conversationId, auth.token);
      }
...
function resolveCustomerDisplayName(customerPhone: string, ...candidates: Array<string | null | undefined>) {
...
function isUsableCustomerName(value: string | null | undefined, customerPhone: string) {
  if (!value || !value.trim()) {
    return false;
  }
