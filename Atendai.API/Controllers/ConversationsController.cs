...
public sealed class ConversationsController(IConversationService conversationService) : ControllerBase
{
    ...
    public async Task<IActionResult> GetById(Guid conversationId, CancellationToken cancellationToken)
    {
        ...
        var conversation = await conversationService.GetConversationByIdAsync(tenantId.Value, conversationId, cancellationToken);
        return conversation is null ? NotFound(new { message = "Conversa nao encontrada." }) : Ok(conversation);
    }
